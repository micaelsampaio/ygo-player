...
export interface Command {
    ...
    exec(): void
    undo(): void
    toJSON<T extends any>(): { type: string, data: T }
}
...
export interface DrawFromDeckCommandData {
    player: number,
    numberOfCards?: number // default is 1
}
...
export interface ActivateCardCommandData {
    player: number
    id: number
    originZone?: FieldZone // use only in cases like spells it goes from hand to field then activate // to make the effect to go from hand to field
    zone: FieldZone
}
