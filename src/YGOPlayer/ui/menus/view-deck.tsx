...
export function ViewDeckPopup({ duel, deck, visible = true }: { deck: Deck, duel: YGODuel, visible: boolean }) {
    ...
    const cardsToShow = useMemo(() => {
        ...
        return cards.filter(card => {
            ...
            if (typesOfCards.monster && YGOGameUtils.isMonster(card)) showCard = true;
    ...
    >
        ...
        <div className="game-popup-dialog ygo-menu-view-main-deck" onClick={e => {
            e.preventDefault();
            e.stopPropagation();
        }}>
            ...
            <div className="game-popup-content-no-scroll ygo-flex ygo-gap-2 ygo-items-center ygo-pt-0">
                <div>
                    <input className="ygo-menu-view-main-deck-search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search a Card..." />
                </div>
                <div>
                    <CardIcon selected={typesOfCards.monster} type="monster" onClick={() => {
                ...
                <div>
                    <CardIcon selected={typesOfCards.spell} type="spell" onClick={() => {
                        setTypesOfCards(currentState => {
                ...
                <div>
                    <CardIcon selected={typesOfCards.trap} type="trap" onClick={() => {
                        setTypesOfCards(currentState => {
...
function CardIcon({ type, selected, onClick }: { selected: boolean, type: "monster" | "spell" | "trap", onClick?: any }) {
    const className = type === "spell" ? "ygo-spell" : type === "trap" ? "ygo-trap" : "ygo-monster";
    return <div onClick={onClick}>
        <div className={`card-icon ygo-cursor-pointer ${className} ${selected ? `` : 'not-selected'}`}></div>
    </div>
}