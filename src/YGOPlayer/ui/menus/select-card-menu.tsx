...
export function SelectedCardMenu({
  ...
  useLayoutEffect(() => {
    ...
    const selectedCardHtml =
      document.querySelector<HTMLDivElement>(".ygo-card")!;
    ...
    >
      ...
      <div className={`ygo-player-${card.owner}-bg-bottom ygo-flex`}>
        ...
        <div>
          {cardData.isMonster && (
            <>
              <div>LVL: {card.linkval || card.level}</div>
              <div>ATK: {card.currentAtk}</div>
              <div>
                {!cardData.isLinkMonster && <>DEF: {card.currentDef}</>}
              </div>
            </>
          )}
        </div>
      ...
      <div className="mb-2 ygo-text-sm">{cardData.type}</div>
