...
export function ydkToJson(ydkContent: string): DeckData {
    ...
    const mainDeckStart: number = lines.findIndex((line: string) => line.includes("#main")) + 1;
    const extraDeckStart: number = lines.findIndex((line: string) => line.includes("#extra")) + 1;
