...
export function ComboChooseDeck({ deckId, onChoose }: { deckId: string, onChoose: any }) {
    ...
    useEffect(() => {
        const deck: any = JSON.parse(window.localStorage.getItem(selectedDeck) || '{}');
        let id = 0;
    ...
    const handleDrop = (e: React.DragEvent, zoneType: string, index: number) => {
        e.preventDefault();
        const draggedCard = JSON.parse(window.localStorage.getItem("draggedCard") || "{}");

        const mainDeck = deck.mainDeck.filter(card => card.index !== draggedCard.index);
        const extraDeck = deck.extraDeck.filter(card => card.index !== draggedCard.index);
...
export const CardComponent = ({ card }: { card: Card }) => {
    return (
        <CardContainer draggable>
            <CardImage src={`http://localhost:8080/images/cards_small/${card.id}.jpg`} alt={card.name} />
            <CardInfo>
