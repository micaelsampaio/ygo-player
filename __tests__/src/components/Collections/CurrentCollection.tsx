    ...
    <PageContainer>
      <CollectionDetails>
        ...
        <DeckSelectSection>
          ...
          <select
            onChange={(e) => updateCollectionDeck(e.target.value)}
            value={collection.deck.name}
          >
            <option></option>
            {decks.map((deckName) => (
        ...
        {!collection.deck.mainDeck && (
          <DeckWarning>Create a deck component main | extra deck</DeckWarning>
        )}

        <DeckSection>
          <h3>
            Deck: {collection.deck.name} ({collection.deck.mainDeck.length})
          </h3>
        ...
        <CombosSection>
          ...
          {collection.combos.map((combo, index) => (
            <ComboCard key={index}>
              ...
              <ComboActions>
                <ActionButton
                  onClick={() =>
                    createComboImage(collection, combo, { viewer: true })
                  }
                >
                  View Image
                </ActionButton>
                <ActionButton
                  onClick={() =>
                    createComboImage(collection, combo, { download: true })
                  }
                >
                  Create Image
                </ActionButton>
          ...
          <NewComboButton onClick={() => setModal("choose_replay")}>
            Create New Combo
          </NewComboButton>
      ...
      {modal === "choose_replay" && (
        <ChooseReplay close={closeModal} onChange={openSpreadsheetBuilder} />
      )}
    </PageContainer>
  );
}
...
async function createComboImage(
