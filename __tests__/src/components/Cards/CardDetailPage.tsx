...
import { ArrowLeft, Star, ExternalLink, Info, Shield, Award, Database } from "lucide-react";
...
const CardDetailPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"info" | "sets" | "rulings" | "prices">("info");
    ...
    const fetchCardDetails = async () => {
      ...
      try {
        ...
        if (localCards) {
          ...
          cardData = parsedCards.find((c: CardData) => c.id === Number(cardId) || c.id === cardId);
        }
        ...
        setIsFavorite(storedFavorites.some((fav: any) => fav.id === Number(cardId)));
  ...
  const fetchSimilarCards = async (archetype: string) => {
    try {
      const response = await fetch(`https://db.ygoprodeck.com/api/v7/cardinfo.php?archetype=${encodeURIComponent(archetype)}&num=6&offset=0`);
      if (!response.ok) return;
      
      const data = await response.json();
      // Filter out the current card and limit to 5 cards
      const filtered = data.data.filter((c: CardData) => c.id !== Number(cardId)).slice(0, 5);
  ...
  const handleToggleFavorite = () => {
    ...
    if (isFavorite) {
      ...
      newFavorites = storedFavorites.filter((fav: any) => fav.id !== Number(cardId));
    } else {
      // Add to favorites
  ...
  const getHighlightedCardText = (text: string) => {
    ...
    const keywordPatterns = [
      { pattern: /\b(You can only use.*once per turn)\b/g, className: "keyword-restriction" },
      { pattern: /\b(Once per turn[,:])\b/g, className: "keyword-restriction" },
      { pattern: /\b(Normal Summon|Special Summon|Tribute Summon|Flip Summon|Synchro Summon|Xyz Summon|Link Summon|Fusion Summon|Ritual Summon)\b/g, className: "keyword-summon" },
      { pattern: /\b(destroy|negate|banish|control|damage|discard|draw|shuffle|search|add|target|attack)\b/gi, className: "keyword-action" },
      { pattern: /\b(hand|deck|GY|Graveyard|field|banished|Extra Deck)\b/g, className: "keyword-location" },
      { pattern: /\b(ATK|DEF|Level|Rank|Link Rating)\b/g, className: "keyword-stat" },
      { pattern: /\b(EARTH|WATER|FIRE|WIND|LIGHT|DARK|DIVINE)\b/g, className: "keyword-attribute" },
    ];
        ...
        <CardDetailGrid>
          <CardImageSection>
            ...
            <ExternalLinks>
              ...
              <ExternalButton 
                as="a" 
                href={`https://yugipedia.com/wiki/${card.name.replace(/\s+/g, '_')}`}
                target="_blank" 
                rel="noopener noreferrer"
              >
                <Info size={14} />
                ...
                {activeTab === "info" && (
                  <>
                    ...
                    <CardProperty>
                      ...
                      <PropertyValue>{card.race} {card.type}</PropertyValue>
                    </CardProperty>
                    ...
                    {card.linkmarkers && card.linkmarkers.length > 0 && (
                      <CardProperty>
                        ...
                        <PropertyValue>{card.linkmarkers.join(", ")}</PropertyValue>
                      </CardProperty>
                    )}
                    ...
                    {(card.atk !== undefined || card.def !== undefined) && (
                      <CardProperty>
                        ...
                        <PropertyValue>
                          {card.atk !== undefined ? card.atk : '?'} / {card.def !== undefined ? card.def : '?'}
                        </PropertyValue>
                      </CardProperty>
                    )}
                      ...
                      <BanlistInfo>
                        ...
                        <BanlistStatusContainer>
                          {card.banlist_info.ban_tcg && (
                            <BanlistStatus $status={card.banlist_info.ban_tcg.toLowerCase()}>
                              <span>TCG: {card.banlist_info.ban_tcg}</span>
                            </BanlistStatus>
                          )}
                          {card.banlist_info.ban_ocg && (
                            <BanlistStatus $status={card.banlist_info.ban_ocg.toLowerCase()}>
                    ...
                    <CardDescription>
                      ...
                      <DescriptionText 
                        dangerouslySetInnerHTML={getHighlightedCardText(card.desc)}
                      />
                    </CardDescription>
                  </>
                )}
                ...
                {activeTab === "sets" && (
                  <CardSetsContainer>
                    ...
                    {card.card_sets && card.card_sets.length > 0 ? (
                      ...
                      <NoDataMessage>No set information available</NoDataMessage>
                    )}
                  </CardSetsContainer>
                )}
                  ...
                  <RulingsContainer>
                    ...
                    <RulingsList>
                      <RulingItem>
                        ...
                        <RulingText>
                          Rulings data is not available in the API. For detailed rulings, please check the official Yu-Gi-Oh! database or Yugipedia.
                        </RulingText>
                      </RulingItem>
                    </RulingsList>
                    <RulingsDisclaimer>
                      Note: For official and up-to-date rulings, always refer to the official Yu-Gi-Oh! database or contact a certified judge.
                    </RulingsDisclaimer>
                  </RulingsContainer>
                )}
                ...
                {activeTab === "prices" && (
                  <PricesContainer>
                    ...
                    {card.card_prices && card.card_prices.length > 0 ? (
                      ...
                      <NoDataMessage>No price information available</NoDataMessage>
                    )}
                    <PriceDisclaimer>
                      Note: Prices are approximate and subject to change. Last updated: {new Date().toLocaleDateString()}
                    </PriceDisclaimer>
                  </PricesContainer>
                )}
              </TabContent>
          ...
          <RelatedCardsSection>
            ...
            <RelatedCardsGrid>
              {similarCards.map(similarCard => (
                <RelatedCardItem key={similarCard.id} onClick={() => navigate(`/card/${similarCard.id}`)}>
                  <RelatedCardImage 
...
const FavoriteButton = styled.button<{ $isFavorite: boolean }>`
  ...
  background: ${({ $isFavorite }) => $isFavorite ? theme.colors.warning.main : 'rgba(0, 0, 0, 0.5)'};
  color: ${({ $isFavorite }) => $isFavorite ? theme.colors.text.inverted : theme.colors.text.inverted};
  border: none;
  border-radius: 50%;
  width: 40px;
  height: 40px;
  ...
  &:hover {
    opacity: 1;
    transform: ${({ $isFavorite }) => $isFavorite ? 'scale(1.2) rotate(5deg)' : 'scale(1.2)'};
    background: ${({ $isFavorite }) => $isFavorite ? theme.colors.warning.dark : 'rgba(0, 0, 0, 0.7)'};
...
const TabButton = styled.button<{ $active: boolean }>`
  ...
  background: none;
  border: none;
  color: ${({ $active }) => $active ? theme.colors.primary.main : theme.colors.text.secondary};
  font-weight: ${({ $active }) => $active ? theme.typography.weight.bold : theme.typography.weight.medium};
  cursor: pointer;
  transition: all 0.2s ease;
  position: relative;
  ...
  &:after {
    ...
    width: 100%;
    height: 3px;
    background: ${({ $active }) => $active ? theme.colors.primary.main : 'transparent'};
    border-radius: 3px;
  ...
  &:hover {
    ...
    &:after {
      background: ${({ $active }) => $active ? theme.colors.primary.main : theme.colors.primary.light};
      transform: scaleX(1);
    }
  }
...
const BanlistStatus = styled.div<{ $status: string }>`
  ...
  background-color: ${({ $status }) => 
    $status === 'forbidden' ? theme.colors.error.main :
    $status === 'limited' ? theme.colors.warning.main :
    $status === 'semi-limited' ? theme.colors.info.main :
