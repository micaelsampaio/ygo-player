...
// Export analyzeDeck as a standalone function so it can be imported directly by other modules
export const analyzeDeck = (deck: Deck): DeckAnalytics => {
  ...
  console.log(`DEBUG: Deck analysis completed in ${analysisTime.toFixed(2)}ms`);
...
export function useDeckAnalytics() {
  const analyzeDeckCallback = useCallback(analyzeDeck, []);

  return { analyzeDeck: analyzeDeckCallback };
}
