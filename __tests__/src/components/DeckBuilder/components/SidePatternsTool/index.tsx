...
const SidePatternsTool: React.FC<SidePatternsToolProps> = ({ deck, onUpdateDeck }) => {
  // States for the form
  ...
  useEffect(() => {
    ...
    if (formMode === 'create' || !selectedPattern) {
      ...
      const removeCardCounts = convertToCardCounts(selectedPattern.cardsToRemove);
      ...
      const finalRemoveCounts = uniqueMainExtra.map(card => {
        const existingCount = removeCardCounts.find(c => c.card.id === card.id);
  ...
  const handleIncreaseCardRemove = (card: Card) => {
    const deckCardCount = getCardCount([...deck.mainDeck, ...(deck.extraDeck || [])], card.id);
    const currentCount = cardsToRemoveCount.find(c => c.card.id === card.id)?.count || 0;
    ...
    setCardsToRemoveCount(prev => 
      prev.map(c => c.card.id === card.id ? { ...c, count: c.count + 1 } : c)
    );
  ...
  const handleIncreaseCardAdd = (card: Card) => {
    ...
    const currentCount = cardsToAddCount.find(c => c.card.id === card.id)?.count || 0;
  ...
  const handleSavePattern = () => {
    ...
    if (cardsToRemove.length !== cardsToAdd.length) {
      showStatus('The number of cards to side out must match the number of cards to side in.', 'error');
      return;
    }

    const pattern: SidingPattern = {
      id: formMode === 'edit' && selectedPattern ? selectedPattern.id : `pattern-${Date.now()}`,
      name: patternName,
      matchup,
      description,
      cardsToRemove,
      cardsToAdd,
      createdAt: formMode === 'edit' && selectedPattern ? selectedPattern.createdAt : Date.now(),
      updatedAt: Date.now()
    };
  ...
  const handleDeletePattern = () => {
    ...
    if (window.confirm(`Are you sure you want to delete the side pattern "${selectedPattern.name}"?`)) {
  ...
  const handleApplyPattern = () => {
    ...
    try {
      ...
      showStatus('Error applying side pattern: ' + (err instanceof Error ? err.message : 'Unknown error'), 'error');
    }
  };
  ...
  if (isLoading) {
    return <div className="side-patterns-loading">Loading side patterns...</div>;
  }
    ...
    <div className="side-patterns-tool">
      ...
      <div className="side-patterns-header">
        ...
        <p className="side-patterns-description">
          Create and manage side patterns to quickly swap cards for different matchups.
          Each pattern lets you define which cards to remove from your main/extra deck and
          which cards to add from your side deck.
        </p>
      </div>
          ...
          <div className="patterns-actions">
            <button 
              ...
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('Create New Pattern button clicked with explicit event handling');
                handleCreatePattern();
              }}
              onKeyDown={(e) => {
        ...
        <div className="pattern-details-container">
          {isFormVisible ? (
            <div className="pattern-form">
              <h3>{formMode === 'create' ? 'Create New Pattern' : 'Edit Pattern'}</h3>
              ...
              <div className="cards-selection">
                <div className="selection-container">
                  ...
                  <p className="selection-hint">Click + or - to adjust the number of copies</p>
                  <div className="cards-grid">
                    {cardsToRemoveCount.map((cardCount) => {
                      // Count how many copies of this card exist in the deck
                      const maxCount = getCardCount([...deck.mainDeck, ...(deck.extraDeck || [])], cardCount.card.id);
                      
                      return (
                        <div 
                          key={`remove-${cardCount.card.id}`}
                          className={`card-item ${cardCount.count > 0 ? 'selected' : ''}`}
                        >
                          <img 
                            src={cardCount.card.card_images?.[0]?.image_url_small || `https://images.ygoprodeck.com/images/cards_small/${cardCount.card.id}.jpg`}
                          ...
                          <div className="card-counter">
                            ...
                            <span className="counter-value">{cardCount.count}/{maxCount}</span>
                            <button 
                              className="counter-button"
                              onClick={(e) => {
                                e.stopPropagation();
                ...
                <div className="selection-container">
                  ...
                  <div className="cards-grid">
                    {cardsToAddCount.map((cardCount) => {
                      ...
                      const maxCount = getCardCount(deck.sideDeck || [], cardCount.card.id);
              ...
              <div className="form-footer">
                {cardsToAdd.length !== cardsToRemove.length && (
                  <div className="form-warning">
                    Warning: The number of cards to side out ({cardsToRemove.length}) must match 
                    the number of cards to side in ({cardsToAdd.length})
                  </div>
                )}
                
                <div className="form-actions">
                  <button className="cancel-button" onClick={handleCancelForm}>Cancel</button>
                  <button 
                    className="save-button"
                    onClick={handleSavePattern}
                    disabled={!patternName || !matchup || cardsToAdd.length !== cardsToRemove.length}
                  >
                    Save Pattern
                  </button>
                </div>
            ...
            <div className="pattern-details">
              ...
              <div className="pattern-info">
                <div className="info-row">
                  ...
                  <div>{new Date(selectedPattern.createdAt).toLocaleDateString()}</div>
                </div>
                <div className="info-row">
                  <div className="info-label">Last updated:</div>
                  <div>{new Date(selectedPattern.updatedAt).toLocaleDateString()}</div>
                </div>
                <div className="info-row">
